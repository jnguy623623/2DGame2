...
void Emitter::draw() {
	if (drawable) {
		...
		if (haveImage) {
			...
			ofSetColor(ofColor::red);// used to be 0,0,200
...
void Emitter::update() {
	...
	for (int i = 0; i < sys->sprites.size(); i++) { //calls moveSprite from below
...
void Emitter::moveSprite(Sprite *sprite) {
    sprite->pos += -sprite->heading() * sprite->velocity / ofGetFrameRate();
}
